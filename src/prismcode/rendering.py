...
from .contracts import Evidence, ReviewBrief, SourceRef
...
def _source(source: SourceRef) -> str:
    ...
    return f'<a href="{escape(source.url)}">{content}</a>' if source.url else content
...
def render_html(brief: ReviewBrief) -> str:
    ...
    files = "".join(f"<li><code>{escape(path)}</code></li>" for path in review.changed_files) or "<li>Not provided.</li>"
    source = f'<a href="{escape(review.source_url)}">Open source pull request</a>' if review.source_url else "Source URL not provided."
...
h1 {{ margin:8px 0 12px; font-size:32px; }} .intent {{ font-size:17px; line-height:1.6; }} .meta {{ color:#667085; }}
